  ...
  return (
    <div className={`relative mx-auto w-full max-w-[372px] ${className}`}>
      <div className="relative rounded-[3rem] bg-[#0B0E12] p-3 shadow-[0_50px_130px_-30px_rgba(11,23,18,0.65)] ring-1 ring-inset ring-white/10">
        <div className={`relative flex aspect-[9/19.3] flex-col overflow-hidden rounded-[2.35rem] ${dark ? 'bg-app-dark-bg' : 'bg-app-bg'}`}>
          {/* Статус-бар iOS */}
          <div className={`relative z-30 flex shrink-0 items-center justify-between px-7 pt-3.5 ${dark ? 'text-app-dark-text' : 'text-app-ink'}`}>
          ...
          <div className="pointer-events-none absolute inset-x-0 bottom-0 z-30 flex justify-center pb-2">
            <div className={`h-1 w-32 rounded-full ${dark ? 'bg-app-dark-text/25' : 'bg-app-ink/25'}`} />
          </div>
        </div>
      </div>
    </div>
